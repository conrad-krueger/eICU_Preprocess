...
def get_pheno_listfile(path_to_eicu):
    ...
    pats = pd.read_csv(os.path.join(path_to_eicu, 'patient.csv'))
...
def split(name, eicu_path, write=False):
    ...
    len_test = 0
    pats = dataframe_from_csv(os.path.join(eicu_path, 'patient.csv'), index_col=False)
    train_list = dataframe_from_csv(os.path.join(name, 'train_listfile.csv'), index_col=False)
    test_list = dataframe_from_csv(os.path.join(name, 'test_listfile.csv'), index_col=False)
    val_list = dataframe_from_csv(os.path.join(name, 'val_listfile.csv'), index_col=False)
...
def append_ihm(filename, min_itemoffset, los, expired, ihm_listfile):
    HOURS_MINIMUM = 48
    if los >= HOURS_MINIMUM and expired in ["Expired", "Alive"] and min_itemoffset <= 48:
        ihm_listfile['stay'].append(filename)
        ihm_listfile['y_true'].append(1 if expired == "Expired" else 0)

# Append to Decomp listfile iff length-of-stay >= 5 hrs and will append hourly window
def append_decomp(filename, min_itemoffset, los, expired, decomp_listfile):
    HOURS_MINIMUM = 5
...
def append_los(filename, min_itemoffset, los, expired, los_listfile):
    HOURS_MINIMUM = 5
...
def create_dir(task_path, task, listfile, splits=None):
    ...
    if splits is not None:
        ...
        mask = ~global_listfile['stay'].isin(test_union_val)
        global_listfile[mask].to_csv(os.path.join(new_dir, f'train_listfile.csv'), index=False)

        mask = global_listfile['stay'].isin(splits['test'])
        global_listfile[mask].to_csv(os.path.join(new_dir, f'test_listfile.csv'), index=False)

        mask = global_listfile['stay'].isin(splits['val'])
        global_listfile[mask].to_csv(os.path.join(new_dir, f'val_listfile.csv'), index=False)
...
def populate_root(eicu_path, task_list, splits, patients, mapper, min_records=15):
    ...
    for i,(stay, min_time, los, file_length, status) in enumerate(
            zip(patients['stay'], patients['min_time'], patients['los'], patients['file_length'], patients['status'])
        ):
        ...
        for task in task_list:
            append_functions[task](stay, min_time, los, status, listfiles[task])
        if do_pheno:
